"""Hermes Cloud Studio — VM API routes (MERGED-011).

Extraido de hermes_api_v2.py. Por enquanto consolidado em um unico arquivo;
proximas iteracoes podem split-ar por dominio (audit, scraper, outreach,
linkedin, internal). Os helpers (_get_li_db, _li_log, _build_li_config etc)
permanecem aqui por usarem state compartilhado da VM.
"""
...
from vm_core.state import (
    ...
    _touch_campaign_run,
    _track_run_lifecycle,
    get_db,
    logger,
    spawn,
)
from vm_core.models import (
    ActivityCreate,
    AuditBatchRequest,
...
@router.post("/api/prospects/{prospect_id}/outreach")
...
@router.post("/api/outreach/batch")
...
@router.patch("/api/hermes/skills/{skill_name}")
...
def _apply_linkedin_migration():
    ...
    # Project root: vm_api/ -> ../
    sql_path = Path(__file__).resolve().parent.parent / "migrations" / "2026_06_linkedin_full.sql"
...
@router.get("/api/linkedin/session-check")
...
@router.post("/api/internal/li_at_update")
