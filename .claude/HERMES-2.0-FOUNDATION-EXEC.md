...
> **UI-P1 ✅ COMPLETO (2026-06-23)**: basemap real de Cuiabá (.pmtiles tilemaker v3/OSM/Geofabrik), prospects circles data-driven, bairros choropleth ST_MakeValid, region modal frosted-glass, legend filter, reduced-motion gate. Todos os 7 gates verificados em produção.

### Auditoria independente (orquestrador, 2026-06-23) — ✅ + 1 caveat + correção de label
- Verifiquei real: `.pmtiles` MVT zoom 8-14 (5.1M) + style servido com `source: pmtiles:///tiles/cuiaba.pmtiles` + 15 layers (transportation×8/water/building/landuse), **sem oklch** → **basemap de ruas REAL renderiza** (o #1 que o owner cobrou). Prospects 2113 features. reduced-motion: app.js:351 `prefersReduce→jumpTo` senão `flyTo curve:1.42` ✅. 5 Hermes healthy + 26 não-Hermes intactos. Exec caçou 5 bugs reais (tilemaker v3 Find/Layer API, config `settings`, osmium complete_ways, ST_MakeValid bairros, Number() XSS).
- **CAVEAT choropleth (não-blocker, não notado pelo exec)**: o join point-in-polygon casa só **75/2113 prospects** (3.5%) — os polígonos OSM admin_level=10 (194 bairros) NÃO cobrem toda a extensão dos prospects (pontos vão a lng -56.23/lat -15.35; bairros param em -56.14/-15.44). SRID ok (4326), ST_MakeValid ok — é **limitação da fonte de bairros**, não bug. Layer de dots mostra os 2113; só o choropleth é esparso. **Resolver com hexes H3 no UI-P2** (ladrilham toda a área, independem de polígono administrativo) — NÃO insistir em bairros OSM incompletos.
