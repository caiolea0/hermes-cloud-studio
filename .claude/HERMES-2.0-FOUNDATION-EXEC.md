...
- **PageSpeed key pendente (opcional)**: sem `HERMES_PAGESPEED_KEY`, PSI roda em modo graceful (skip) → score usa sinais não-PSI (no_website/social/schema/mobile do web_audit). Setar a key no `.env` da VPS habilita os sinais PSI (perf/seo/a11y).
