...
+- [ ] **Task 11 (F.2.5c) — LiveLogTail standalone** (deferido de F.2.5b): dashboard/components/live_log_tail.js NOVO consumindo handlers stub `daemon.log_event` + `daemon.decision` (F.2.5a console.log → real render). Ring buffer 200 entries cap FIFO, virtual list rendering (DOM nodes só viewport visible), auto-scroll + pause-on-hover, filtros log_level (info/warn/error) + emitter (daemon/loops/api), uso `window.hermesUtils.safeMerge` (utility F.2.5b Step 0), sanitize via DOMPurify para innerHTML += em log items (esperado sanitize count 3 → ~5-7). MATURE touch dashboard/app.js (init em loadMissionControl). frontend-ux-reviewer gate ativa. ~3-4h, 50-70k tokens sessão dedicada.
